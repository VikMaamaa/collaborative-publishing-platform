...
import { usePosts, useOrganizations, useUserInvitations } from '@/lib/api-hooks';
...
export default function DashboardPage() {
  ...
  const { data: publishedPostsData, loading: publishedPostsLoading } = usePosts({ status: 'published' });
  const { data: organizations, loading: orgsLoading } = useOrganizations();
  const { data: invitations, loading: invitationsLoading } = useUserInvitations();

  const isLoading = postsLoading || publishedPostsLoading || orgsLoading || invitationsLoading;
        ...
        <DashboardLayout>
          ...
          >
            ...
            <DashboardStats className="mb-8">
              <DashboardStatCard
                title="Total Posts"
                value={isLoading ? '' : postsData?.total ?? 0}
                change={''}
                changeType="neutral"
                icon={
              ...
              <DashboardStatCard
                title="Organizations"
                value={isLoading ? '' : organizations?.length ?? 0}
                change={''}
                changeType="neutral"
                icon={
              ...
              <DashboardStatCard
                ...
                value={isLoading ? '' : invitations?.length ?? 0}
                change={''}
                changeType="neutral"
                icon={
              ...
              <DashboardStatCard
                ...
                value={isLoading ? '' : publishedPostsData?.total ?? 0}
                change={''}
                changeType="neutral"
                icon={
